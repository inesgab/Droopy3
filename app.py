...
/Users/bacterie/Documents/Experiences/Milli/Exp_Dilution_1_4_16_64_256_1024/CAA/EXP20210219_2158/analysis
"""
...
from PyQt5.QtWidgets import QDialog, QApplication, QPushButton, QVBoxLayout,QTabWidget, QFileDialog, QShortcut, QMessageBox
...
class TabWidget(QTabWidget, Ui_TabWidget):
    ...
    def __init__(self):
        ...
        self.ButtonGoBackOutliers.clicked.connect(lambda : self.spinBoxSelectCheckOutlier.setValue(self.spinBoxSelectCheckOutlier.value()-1))
    ...
    def selectFolder(self):
         ...
         if os.path.exists(analysis_path) and os.path.isdir(analysis_path):
            ...
            self.msgBox.setText("The selected folder contains directly the subfolder 'analysis'. Please select the folder before (containing the experiment folder)")
    ...
    def readCsv(self):
        ...
        for choice in self.parameters.index.values:
            if choice == 'GFP':
                ...
                self.yieldNoGrowthThreshGFP.setText( str(self.parameters['noGrowthThresh'][choice] ))
                self.maxTimeNoGrowthThreshGFP.setText( str(self.parameters['threshOutlayer'][choice] ))
            ...
            if choice == 'RFP':
                ...
                self.yieldNoGrowthThreshRFP.setText( str(self.parameters['noGrowthThresh'][choice] ))
                self.maxTimeNoGrowthThreshRFP.setText( str(self.parameters['threshOutlayer'][choice] ))
            ...
            if choice == 'PVD':
                ...
                self.yieldNoGrowthThreshPVD.setText( str(self.parameters['noGrowthThresh'][choice] ))
                self.maxTimeNoGrowthThreshPVD.setText( str(self.parameters['threshOutlayer'][choice] ))
    ...
    def setParam(self):      
        ...
        timeThreshList = [self.maxTimeGFP.toPlainText(), self.maxTimeRFP.toPlainText(), self.maxTimePVD.toPlainText()]
        threshOutlayerList = [self.yieldNoGrowthThreshGFP.toPlainText(), self.yieldNoGrowthThreshRFP.toPlainText(), self.yieldNoGrowthThreshPVD.toPlainText()]
        noGrowthThreshList = [self.maxTimeNoGrowthThreshGFP.toPlainText(), self.maxTimeNoGrowthThreshRFP.toPlainText(), self.maxTimeNoGrowthThreshPVD.toPlainText()]
        yieldNoGrowthThresh = [self.yieldNoGrowthThreshGFP.toPlainText(), self.yieldNoGrowthThreshRFP.toPlainText(), self.yieldNoGrowthThreshPVD.toPlainText()]
        startTimeList = [self.startTimeGFP.toPlainText(), self.startTimeRFP.toPlainText(), self.startTimePVD.toPlainText()]
        calibList = [[float(self.calibAGFP.toPlainText()),float(self.calibBGFP.toPlainText())], [float(self.calibARFP.toPlainText()),float(self.calibBRFP.toPlainText())],[float(self.calibAPVD.toPlainText()),float(self.calibBPVD.toPlainText())]]
        

        d = dict({'rootPath' : self.rootPathEdit.toPlainText(),
             'channelList': [ channelList[i] for i,l in enumerate(choice) if l],
             'incCarte' : float(self.incCarte.toPlainText()),
             'timeThresh' : [ float(timeThreshList[i]) for i,l in enumerate(choice) if l],
             'threshOutlayer' : [ float(noGrowthThreshList[i]) for i,l in enumerate(choice) if l],
             'display' : self.displayPlot.isChecked(),
             'noGrowthThresh' : [ float(yieldNoGrowthThresh[i]) for i,l in enumerate(choice) if l],
             'startTime' : [ float(startTimeList[i]) for i,l in enumerate(choice) if l],
             'deletionData' : self.deleteResults.checkState(),
             'ymin' : [ float(self.yMinPlot.toPlainText()) for i,l in enumerate(choice) if l],
             'ymax' : [float(self.yMaxPlot.toPlainText()) for i,l in enumerate(choice) if l],
             'nbReps' : 0,
    ...
    def getBackOutlier(self):
        ...
        self.plot(self.checkOutliersLabel, self.listPlotFitName, nbFit, self.nameCheckOutliers)
    ...
    def checkOutliers(self):
        ...
        for self.channel in self.parameters.index:
            ...
            folder=sorted([join(join(self.rootPath,o),'analysis/') for o in listdir(self.rootPath) if (o[:3]=='201' or o[:3]=='EXP') and os.path.isdir(os.path.join(self.rootPath, o))])
            self.listPlotFitName=[]
            for self.pathFolder in folder:
                
                pathRlt = self.pathFolder + 'resultIndiv/'
                dirlist = [filename for filename in sorted(os.listdir(pathRlt)) if os.path.isdir(os.path.join(pathRlt,filename))]
           
                if os.path.exists(self.pathFolder + 'outliersbyEye.csv'):
                    now = str(datetime.datetime.now())[:19]
                    now = now.replace(":","_")
                    shutil.move(self.pathFolder +'outliersbyEye.csv', self.pathFolder +'outliersbyEye_save'+now+'.csv')
                    print('copied outliersbyEye.csv as outliersbyEye_save'+now+'.csv')
                ...
                if os.path.exists(self.pathFolder+'outliersbyAlgo_'+self.channel+'.csv'):
                    fileOutliersAlgo = pd.read_csv(self.pathFolder+'outliersbyAlgo_'+self.channel+'.csv', header=None)
            ...
            self.plot(self.checkOutliersLabel, self.listPlotFitName, 0, self.nameCheckImage)
    ...
    def writeOutliers(self, outliers):  
        ...
        self.progressBarOutliers.setValue(np.int_(100*nbFit/len(self.listPlotFitName)))
        app.processEvents()
        nbFit +=1
    ...
    def checkData(self):
        ...
        for channel in self.parameters.index:
            ...
            for path in folder:
                ...
                checkfitDataCalib(df, dropMap, label, channel, self.rootPath+'/'+path.split('/')[-3],self.parameters)
    ...
    def displayFit(self):   
        ...
        if self.loadedFit == False:
            for self.channel in self.parameters.index:
                ...
                self.plot(self.checkFitLabel, self.listPlotFitName, 0, self.nameCheckFit)
                self.loadedFit = True
                
        else : 
            nbImage = self.spinBoxSelectCheckFit.value()
            self.plot(self.checkFitLabel, self.listPlotFitName, nbImage, self.nameCheckFit)
