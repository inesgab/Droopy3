...
module_path = os.path.abspath(os.path.join('/Users/inesgabert/Documents/LBE/Droopyv2/fitderivpackage1.02'))
...
#try:
    ...
    if 'CAA' == p[posOfFolderInSource] or  'CAAnoGly' == p[posOfFolderInSource] or 'CAA1' == p[posOfFolderInSource] or 'CAAspp' == p[posOfFolderInSource] or 'AyshaCAA1' == p[posOfFolderInSource]:
    ...
    if 'Exp_GreenRed' == p[-6] : #experiment green red to measure the yield as a function of ratio
        c = label.split('g')
        b = c[1].split('-')
        val= float(b[0]) 
...
def getfitData(df, j, channel,startTime,timeThresh,threshOutlayer, incCarte,noGrowthThresh): 
    """
    ...
    try:
        ...
        [x, y, idxThresDetect]= dataUpAndDown(df, j, channel, seuilDetection) #return the time serie with two other series corresponding to the data plus or minus the cov
        ...
        if channel=='PVDoverGFP':
            ...
            idxThresDetectGFP=bisect.bisect(p.fluo_3_median,seuilDetection)#find threshold of data above detection 5e-3V
            idxThresDetectPVD=bisect.bisect(p.fluo_1_median,seuilDetection)
            
            idxThresDetect= max(idxThresDetectPVD,idxThresDetectGFP)
            
            yFluo=np.array(np.log(p['fluo_3_area']*p['speed']/p['size']),dtype=np.float64) #to screen for nonoutlayer
        ...
        #print('idxStart= ',idxStart, ' idxThres= ',idxThres)
        if len(y[:,0])>2:
            if len(y[:,0])>idxOutlayer :
                if channel=='GFP'or channel=='PVD' or channel=='RFP':
                    ...
                    nonoutlayer= yFluo[idxOutlayer]>yFluo[2]+1 or threshOutlayer<=0 
...
def dataUpAndDown(df, j, channel, seuilDetection):
    Nb = ''
    print('dataUpAndDown drop:'+str(j))
    if channel=='RFP':
        Nb ='2'
    elif channel=='GFP':
        Nb ='3'     
    ...
    if Nb!='':
        ...
        yUp = np.array(y * ( 1 + fluo_cov) ) #the data cov are the std/mean for the channel
        ...
        idxThresDetect=bisect.bisect(y,seuilDetection)#find threshold of data above detection 5e-3V


        print('raw_dataUpAndDown')
        print('drop '+str(j))
        print('yUp')
        print(yUp)
        print('yDown')
        ...
        # x = np.delete(x,idxNan)
        # y = np.delete(y,idxNan)
        # yUp = np.delete(yUp,idxNan) 
        # yDown = np.delete(yDown,idxNan) 
        # yDown[yDown<0]=threshArea

        #to have a nice fit, set to the value that are below the detection threshold to the detection threshold 


        print('idxThresDetect')
        print(idxThresDetect)

        if idxThresDetect>0:
            indices = yUp <= y[idxThresDetect] #bug ici
            print('indices')
            print(indices)
        ...
        try :
            ...
            print('logy', type(y1))
            print(y1)
            print('logyUp', type(y2))
            print(y2)
            print('logyDown', type(y3))
            print(y3)
        ...
        x_og[sorted_order] = x
        y1_og = np.empty_like(y1)
        y1_og[sorted_order] = y1
        y2_og = np.empty_like(y2)
        y2_og[sorted_order] = y2
        y3_og = np.empty_like(y3)
        y3_og[sorted_order] = y3

        print('old_results', x, np.column_stack((y1,y2,y3)))
        print('new_results', x_og, np.column_stack((y1_og,y2_og,y3_og)))
        return [x_og, np.column_stack((y1_og,y2_og,y3_og)), idxThresDetect]
    else:
            ...
            if content==dataFile: 
                ...
                if flag==True:
                    ...
                    if channel=='SpeedSize':   
                        ...
                        ax.title(date[-3]+' '+dataFile+' std/m='+str("{0:.2g}".format(np.mean(err)))+'%')
        ...
        for j,content in enumerate(dropMap[:,1]):
            ...
            if content==dataFile: 
                ...
                if flag==True:
                    ...
                    y = (y * calibParam[0] + calibParam[1])/np.log(10) #get the log of concentration in log10
                    ...
                    ax.plot(x, y[:,0],color=c, linewidth=.5)
                    #ax.errorbar(x, y, ystd,  fmt='.r')
                else: 
                    print("data removed by thresh drop="+str(j))
...
#                         combined_csv = pd.concat( [combined_csv, pddf(statdict,index=[0])])
#                         #print(combined_csv)
...
#                     combined_csv = pd.concat( [combined_csv, pddf(
#                     {'max df': np.nan,
#                      'max df std': np.nan,
...
def fitDataIndivParallel(df, dropMap, label, channel, folder, parameters):
    ...
    if os.path.exists(folder + 'outliersbyAlgo_'+channel+'.csv') and not parameters['fromLag'][channel]:
        os.remove(folder + 'outliersbyAlgo_'+channel+'.csv')
    elif not os.path.exists(folder + 'outliersbyAlgo_'+channel+'.csv') and not parameters['fromLag'][channel]:
        with open(folder + 'outliersbyAlgo_'+channel+'.csv', 'a') as f:
            writer = csv.writer(f)
            writer.writerow([-1])
                ...
                if flag==True:
                    ...
                    try:
                        ...
                        if display==True:
                            ...
                            plotDerivCurve(j, np.column_stack((x,x,x)), y, [2*(y[:,1]-y[:,0])] , q, startTime, timeThresh, pathFitPlot, dataFile, channel, ymin,ymax, calibParam, incCarte, content, fromLag, display)
...
def plotDerivCurve(j, x, y, ystd, q, startTime, timeThresh, pathFitPlot, dataFile, channel, ymin, ymax, calibParam, incCarte, content, fromLag, display=False):
   ...
   # ystd = ystdlist[0]
    ...
    ax.set_title(dataFile+"\n start fit after detection threshold= "+ str(incCarte)+ '\n drop='+ str(j))
    ax.set_xlim(0,timeThresh)
    ...
    if display==True:
        if fromLag:
            fig.savefig(pathFitPlot+dataFile+'fitAndDerivativeIndivFromDetectionTime_Drop'+str(j)+channel+'.png',bbox_inches='tight')
        else:
            fig.savefig(pathFitPlot+dataFile+'fitAndDerivativeIndiv_Drop'+str(j)+channel+'.png',bbox_inches='tight')
...
def wrapFitLoop(parameters,rootPath, path, channel):
      ...
      checkfitData(df, dropMap, label, channel,rootPath+path.split('/')[-3],parameters)
...
def getDataHistogram(label,path,channel,fromLag):
    ...
    for n,labelName in enumerate(label):
        if fromLag : 
            df = pd.read_csv(folder + labelName+'resultIndivFitFromDetectionTime_Drop_'+channel+'.csv'  )
            ht = pd.read_csv(folder+labelName+'halftime'+channel+'.csv')
            ht.columns = ['drop', labelName+'_ht']
            ht = ht.set_index('drop')
        else:   
            df = pd.read_csv(folder + labelName+'resultIndiv_Drop_'+channel+'.csv'  )
            ht=pddf()
...
def poolDataInterpolate(dropMap, df, label,channel, path, parameters, savefig): 
    ...
    for j,tmp in enumerate(label):
        ...
        p=df[len(df)-4]#antepenultien run pour creer le range de temps sur lequel interpoler
        xLast=np.array(p.time/3600,dtype=np.float64)
        #x2=np.arange(.5, min(max(x0)-min(x0),max(xLast)-min(xLast)), .5)
        xinterp=np.arange(0, timeThresh, .5)
        ...
        for j,content in enumerate(dropMap[:,1]):
            ...
            if content==tmp:    
                ...
                [x,yyy,flag] = getfitData(df, j, channel,startTime,timeThresh,threshOutlayer, incCarte,noGrowthThresh) 
              
                if flag==True:
                    try:
                        y = yyy[:,0]
                        f = si.interp1d(x, y)
                        x2 = np.arange(np.ceil(x[0]), np.floor(x[-1]), .1)
                        y2=f(x2)
                        dtmp  = pddf()
                    ...
                    if savefig == True:
                        ...
                        plt.savefig(pathgrowth+'drp'+str(j)+'_deriveAll_nbpt'+'_'+tmp, format='png',bbox_inches='tight')
                        plt.show()
                else:
...
#        plt.ylim([-10,-2])
        ...
        plt.ylabel('log(fluo)')
        plt.show()
        fig.savefig(folder+dataFile+'checkDataInterp'+channel+'.png',bbox_inches='tight')
...
def getThresHalfTime(rootPath,source,label):
    if rootPath==source + 'Exp_Dilution_1_4_16_64_256_1024/CAA/' or rootPath==source + 'Exp_Dilution_1_4_16_64_256_1024/CAAnoGly/' or rootPath==source + 'Exp_Dilution_1_4_16_64_256_1024/CAA1/' or rootPath==source + 'Exp_Dilution_1_4_16_64_256_1024/AyshaCAA1/' or rootPath==source + 'Exp_Dilution_1_4_16_64_256_1024/CAAspp/':
    ...
    if rootPath==source + 'Exp_Dilution_1_4_16_64_256_1024/M9gly' or rootPath==source + 'Exp_Dilution_1_4_16_64_256_1024/M9glyFromCAA/' :
    ...
    if rootPath == source + 'Exp_BipyCAA_WT/'or rootPath == source + 'Exp_BipyCAA_PvdS/':
        thresh=3
