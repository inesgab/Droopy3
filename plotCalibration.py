...
for runToConsider in range(8):
    ...
    ax.set_xlim([-6, 10])
    fig.savefig(
        rootPath + "calibration_PMT" + channel + f"_run{runToConsider+1}"+ ".pdf", format="pdf", bbox_inches="tight"
    )
    ...
    calibrationTable.to_csv(rootPath + f"calib_run{runToConsider +1}.csv")